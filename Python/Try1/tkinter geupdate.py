pip from threading import Timer
from tkinter import *
...
class NumPad():
    def __init__(self, master):
        self.beweeg = 0
        self.t = Timer(10.0, self.alarmAf)                                #Creeert een timer die na 10 seconden de functie alarmAf() uitvoert.
    ...
    def beweging(self):
        ...
        status = self.leesStatus()
        if status == 1:
                self.t.start()                                               #Start de timer van 10 seconden om de alarmcode in te voeren
