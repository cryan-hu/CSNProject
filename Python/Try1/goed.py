...
class NumPad():
    def __init__(self, master):
        ...
        self.knipper = NONE
        self.knipper2 = NONE
        self.status = self.leesStatus("status.txt")
        self.fontToetsen = ('Courier',16)
        self.fontTekst = ('Courier',10)
        self.wwen = []
        self.gebruikers = []
        self.snelheid = IntVar()
        self.alarmSnelheid = IntVar()
        self.alarmSnelheid.set(self.leesStatus("snelheid.txt"))
        self.groen_pin = 27
        self.rood_pin = 22
        self.deur_pin = 17
    ...
    def createFrames(self):
        ...
        self.inputBox = Entry(frame, font=('Courier',30),show="*",textvariable=self.entrybox).pack(padx=14,fill=X)
    ...
    def leesStatus(self,naam):                                           #Leest het status.txt bestand
        with open(naam,'r') as file:
            status = int(file.read(1))
            return status

    def writeStatus(self,naam,a):                                         #Schrijft het status.txt bestand
    ...
    def enter(self,getal):
        ...
        nu = self.entrybox.get()
        status = self.leesStatus("status.txt")
        if nu == '1234':
            if status == 0:
                if self.loopt == False:
    ...
    def rood_snel_aan(self):
        io.output(self.rood_pin,True)
        self.knipper = root.after(self.alarmSnelheid,self.rood_snel_uit)

    def rood_snel_uit(self):
        io.output(self.rood_pin,False)
        self.knipper = root.after(self.alarmSnelheid,self.rood_snel_aan)
